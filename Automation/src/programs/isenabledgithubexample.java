package programs;

import java.time.Duration;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;

public class isenabledgithubexample {

	public static void main(String[] args) throws Exception {
		WebDriver w = new ChromeDriver();
		w.manage().window().maximize();
		w.get("https://github.com/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F&source=header-home");
		w.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
		
		w.findElement(By.id("email")).sendKeys("qst@gmail.com");
		Thread.sleep(2000);
		WebElement continuebtn = w.findElement(By.xpath("//*[@id=\"email-container\"]/div[2]/button"));
		boolean enable = continuebtn.isEnabled();
		if (enable) {
			System.out.println("continue button is enabled");
		} else {
			System.out.println("continue button is disabled");
		}
		//w.findElement(By.id("id=\"email\""))
		
		
	}

}
